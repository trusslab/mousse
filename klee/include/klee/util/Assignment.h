...
public:
    ...
    Assignment(const Assignment &assignment)
        : allowFreeValues(assignment.allowFreeValues), bindings(assignment.bindings),
          expressionCache(assignment.expressionCache), updateListCache(assignment.updateListCache),
          cacheHits(assignment.cacheHits), cacheMisses(assignment.cacheMisses) {
    }
