...
bool Solver::getInitialValues(const Query &query, const std::vector<const Array *> &objects,
	...
	//Yingtong
	//fprintf(stderr, "%s [1]: Solver\n",__FUNCTION__);
    bool hasSolution;
	//Yingtong
	//fprintf(stderr, "%s [2]: Solver\n",__FUNCTION__);
	//Yingtong
	//assert(impl);
...
//         for (std::vector<unsigned char>::const_iterator j = values[i].begin(); j != values[i].end(); ++j)
//                 std::cout << "get concrete data = "<< (int)*j << "\n";
//    }
         //Yingtong end
    // FIXME: Propogate this out.
	//Yingtong
	//fprintf(stderr, "%s [3]: Solver has solution = %d\n",__FUNCTION__, hasSolution);
    if (!hasSolution)
        return false;
...
//Yingtong
//	fprintf(stderr, "%s test getInitialValues[2]\n",__FUNCTION__);
...
bool ValidatingSolver::computeInitialValues(const Query &query, const std::vector<const Array *> &objects,
    ...
    bool answer;
	//Yingtong
	//fprintf(stderr, "%s [1]: ValidatingSolver\n",__FUNCTION__);
...
public:
    ...
    bool computeInitialValues(const Query &, const std::vector<const Array *> &objects,
                              std::vector<std::vector<unsigned char>> &values, bool &hasSolution) {
	//Yingtong
	//fprintf(stderr, "%s [1]: DummySolverImpl\n",__FUNCTION__);
        ++stats::queries;
        ++stats::queryCounterexamples;
        return false;
    }
