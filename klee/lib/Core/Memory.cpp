...
void ObjectState::setConcreteStore(uint8_t *source_addr)
{
    concreteStore->set(source_addr);
}
...
ref<Expr> ObjectState::read32(unsigned offset) const {
    if (!object->isSharedConcrete) {
        if (isConcrete(offset, 4)) {
            return ConstantExpr::create(concreteStore->get()[offset + storeOffset], Expr::Int32);
        } else {
            ref<Expr> Res(0);
        ...
        return ConstantExpr::create(((uint32_t *) object->address)[offset], Expr::Int32);
    }
}
...
void ObjectState::writeEvaluatedValue8(unsigned offset, uint8_t value) {
     concreteStore->get()[offset + storeOffset] = value;
}
void ObjectState::writeEvaluatedValue32(unsigned offset, uint32_t value) {
     concreteStore->get()[offset + storeOffset] = value;
}
void ObjectState::mark8(unsigned offset) {
     setKnownSymbolic(offset, 0);
...
void ObjectState::write(ref<Expr> offset, ref<Expr> value) {
	...
	fprintf(stderr, "%s: write to symbolic offset with symbolic value, how to deal with this in Mousse FIXME\n",__FUNCTION__);
