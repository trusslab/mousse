...
MemoryObject *Executor::addSymRegsExternalObject(ExecutionState &state, void *addr, unsigned size, bool isReadOnly,
	...
	os->setConcreteStore((uint8_t *)addr);
    fprintf(stderr, "%s [2]: mo->address = %p, mo->size = %u, os = %p, os->getConcreteStore = %p\n", __FUNCTION__, (void *)mo->address, mo->size, os, os->getConcreteStore(1));
    if (isReadOnly)
        os->setReadOnly(true);
    return mo;
}
MemoryObject *Executor::addRamExternalObject(ExecutionState &state, void *addr, unsigned size, bool isReadOnly,
                                          bool isUserSpecified, bool isSharedConcrete, bool isValueIgnored) {
/* Check if the MemoryObject with the same hostaddr has been allocated. if it does, return. */
	if((state.addressSpace.findObject((uint64_t)addr)).first) {
		return NULL;
	}
    ...
    mo->isSharedConcrete = isSharedConcrete;
    mo->isValueIgnored = isValueIgnored;
//The bindObjectInState will first check if the hostaddress in mo has corresponding ObjectState. If it does, delete the old ObjectState instance. 
    ObjectState *os = bindObjectInState(state, mo, false);
/* Use guest virtual memory region as ConcreteBuffer. */
	os->setConcreteStore((uint8_t *)addr);
    if (isReadOnly)
        os->setReadOnly(true);
    return mo;
...
void Executor::updateGlobalEnv(ExecutionState &state, void *envptr) {
    ref<ConstantExpr> addr = ConstantExpr::create((uint64_t) envptr, sizeof(void *) * 8);
    llvm::GlobalValue * global_env = NULL;
    unsigned int i = 5;
    global_env = dyn_cast<GlobalValue>(kmodule->constants[i]);
    assert(!(global_env->getName().str().compare("env")));
...
// Disabled, we don't want to promote use of live externals.
    ...
    for (Module::const_global_iterator i = m->global_begin(), e = m->global_end(); i != e; ++i) {
     ...
     //       llvm::errs() << "initializeGlobals [2]: name = " << i->getName() << "\n";
...
Executor::StatePair Executor::concolicFork(ExecutionState &current, ref<Expr> condition, bool isInternal,
                                           bool keepConditionTrueInCurrentState, bool isSyscallConcretize) {
    ...
    if (current.forkDisabled) {
        if (conditionIsTrue) {
            if(isSyscallConcretize) {
                if(!mousseConcretizeWOConstraints) {
...
void Executor::notifyForkBeforeBranch(ExecutionState &originalState, ref<Expr> &condition, Executor::StatePair &targets, int type, bool &swap) {
    // Should not get here
    assert(false && "Must go through S2E");
}
...
ref<klee::ConstantExpr> Executor::toConstant(ExecutionState &state, ref<Expr> e, const char *reason) {
    ...
    if (ki && ki->inst) {
        os << "thread = " << hexval((unsigned long)pthread_self()) << " silently concretizing "<< "(instruction: " << ki->inst->getParent()->getParent()->getName().str() << ": " << *ki->inst << ") ";
    }
    ...
    switch (i->getOpcode()) {
        ...
        case Instruction::Br: {
            ...
            if (bi->isUnconditional()) {
                ...
                // Let plugins decide which branch direction to take
                bool swap = false;
                notifyForkBeforeBranch(state, cond, branches, 1, swap);
                if (branches.first) {
                    if (!swap)
...
#ifdef DEBUG_INST
		    llvm::errs() << ki->inst->getParent()->getParent()->getName().str() << " : Instruction: " << *(ki->inst) << '\n';
#endif
            ref<Expr> base = eval(ki, 0, state).value;
#ifdef DEBUG_INST
		    llvm::errs() << "Instruction::Load base:" << base << '\n';
#endif
...
#ifdef DEBUG_INST
		    llvm::errs() << "Instruction::Store base:" << base << '\n';
#endif
            ref<Expr> value = eval(ki, 0, state).value;
            if(isa<ConstantExpr>(base)) {
                ref<ConstantExpr> test_result = dyn_cast<ConstantExpr>(base);
                assert(!test_result.isNull());
                if(test_result->getZExtValue() == 0x4080558c) {
		            llvm::errs() << ki->inst->getParent()->getParent()->getName().str() << " : Instruction: " << *(ki->inst) << '\n';
		            llvm::errs() << "Instruction::Store base:" << base << '\n';
		            llvm::errs() << "Instruction::Store value:" << value << '\n';
                }
            }
#ifdef DEBUG_INST
		    llvm::errs() << "Instruction::Store value:" << value << '\n';
...
uint64_t Executor::toMultiConstant(ExecutionState &state, ref<Expr> e, const char *reason) {
        ref<Expr> arg;
        if(mousseConcretizeWOConstraints) {
        /* don't do concretization, just choose one possible concrete value */
            fprintf(stderr, "%s: Mousse chose one possible concrete value without adding any constraints\n",__FUNCTION__);
            arg = toConstantSilent(state, e);
        } else {
            fprintf(stderr, "%s: Mousse chose one possible concrete value with its constraints\n",__FUNCTION__);
            arg = toUnique(state, e);
        }
        if (ConstantExpr *CE = dyn_cast<ConstantExpr>(arg)) {
            ...
            if (concolicMode) {
                ...
                assert(0 && "toMultiConstant: Not tested for symbolicmode\n");
            ...
            assert(sp.first == &state);
            bool swap = false;
            notifyForkBeforeBranch(state, condition, sp, 3, swap);
            ...
            if (!swap) {
                ...
                sp.first->pc = savedPc;
                return concreteArg->getZExtValue(); 
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target, Function *function,
    ...
    unsigned i = 1;
    char arg_0[64];
    bool is_external_syscall = false;
    if(ConstantExpr *arg0 = dyn_cast<ConstantExpr>(arguments[0]))
    {
        arg0->toMemory((void *)arg_0);
        if(!strcmp(arg_0, "externalSyscall"))
            is_external_syscall = true;
    }
    if(is_external_syscall) { 
    		llvm::errs() << "Thread = " << hexval((unsigned long)pthread_self()) << " external syscall function:" << function->getName() << "\n";
        for (std::vector<ref<Expr>>::iterator ai = arguments.begin(), ae = arguments.end(); ai != ae; ++ai, ++i) {
            if (AllowExternalSymCalls) { // don't bother checking uniqueness
                ...
                if(mousseConcretizeWOConstraints) {
                    fprintf(stderr, "%s: Mousse chose one possible concrete value without adding any constraints\n",__FUNCTION__);
                    arg = toConstantSilent(state, *ai);
                } else {
                    /* arg will be ConstantExpr only if the concretized value is unique based on the constraints, otherwise arg is same with *ai */
                    fprintf(stderr, "%s: Mousse chose one possible concrete value with its constraints\n",__FUNCTION__);
                    arg = toUnique(state, *ai);
                }
                if (ConstantExpr *CE = dyn_cast<ConstantExpr>(arg)) {
                    ...
                    klee::ref<klee::Expr> condition = EqExpr::create(concreteArg, arg);
    		        llvm::errs() << "Thread = " << hexval((unsigned long)pthread_self()) << " function:" << function->getName() << "\n";
    		        llvm::errs() << "Original arg:" << arg << "\n";
    		        llvm::errs() << "Create condition:" << condition << "\n";
    		        llvm::errs() << "Choose concreteArg:" << concreteArg << "\n";
                    StatePair sp = fork(state, condition, true, true, true);
    
                    assert(sp.first == &state);
                    bool swap = false;
                    notifyForkBeforeBranch(state, condition, sp, 2, swap);
        ...
        for (std::vector<ref<Expr>>::iterator ai = arguments.begin(), ae = arguments.end(); ai != ae; ++ai, ++i) {
            if (AllowExternalSymCalls) { // don't bother checking uniqueness
                ...
                /* we don't fork states for concolic args passed to external functions/syscalls in Mousse, why??*/
    ...
    if (!success) {
	if(!strcmp(function->getName().data(),"fprintf"))
		return;
...
void Executor::writeAndNotify(ExecutionState &state, ObjectState *wos, ref<Expr> &address, ref<Expr> &value) {
    ...
    if(isa<ConstantExpr>(address)) {
        ref<ConstantExpr> concreteAddress1 = dyn_cast<ConstantExpr>(address);
    }
    ...
    if (success) {
        ...
        if (!isWrite) {
            if(isa<ConstantExpr>(result)) {
                ref<ConstantExpr> test_result = dyn_cast<ConstantExpr>(result);
                assert(!test_result.isNull());
            } else {
#ifdef DEBUG_MEM_OP
                if(isa<ConstantExpr>(address)) {
                    ref<ConstantExpr> concreteAddress1 = dyn_cast<ConstantExpr>(address);
                    fprintf(stderr, "%s: concreteAddress = %llx, result is not constant->\n", __FUNCTION__, concreteAddress1->getZExtValue());
                    llvm::errs() << result << "\n";
                }
#endif
            }
