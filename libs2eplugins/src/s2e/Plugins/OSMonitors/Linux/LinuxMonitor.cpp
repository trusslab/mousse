...
//void LinuxMonitor::handleSyscall(S2EExecutionState *state, const S2E_LINUXMON_COMMAND &cmd) {
//    onSyscall.emit(state, cmd.Syscall);
//}
...
//        case LINUX_SYSCALL:
//            handleSyscall(state, cmd);
//            break;
    }
}

void LinuxMonitor::instrumentSyscalls(S2EExecutionState *state) {
    /* This is for syscall monitoring */
    s2e()->getCorePlugin()->onTranslateSoftInterruptStart.connect(
        sigc::mem_fun(*this, &LinuxMonitor::onTranslateSoftInterruptStart));

    s2e()->getCorePlugin()->onTranslateSpecialInstructionEnd.connect(
        sigc::mem_fun(*this, &LinuxMonitor::onTranslateSpecialInstructionEnd));
}

void LinuxMonitor::onTranslateSpecialInstructionEnd(ExecutionSignal *signal, S2EExecutionState *state,
    ...
    signal->connect(sigc::mem_fun(*this, &LinuxMonitor::onSyscallInst));
}

void LinuxMonitor::onTranslateSoftInterruptStart(ExecutionSignal *signal, S2EExecutionState *state,
                                                 TranslationBlock *tb, uint64_t pc, unsigned vector) {
    if (vector != 0x2e) {
        return;
    }

    signal->connect(sigc::mem_fun(*this, &LinuxMonitor::onSyscallInt));
}

void LinuxMonitor::onSyscallInst(S2EExecutionState *state, uint64_t pc) {
...
void LinuxMonitor::onSyscallInt(S2EExecutionState *state, uint64_t pc) {
//    target_ulong syscallid = state->regs()->read<target_ulong>(CPU_OFFSET(regs[R_EAX]));
...
void LinuxMonitor::processSyscall(S2EExecutionState *state, uint64_t pc, uint64_t syscallId, uint64_t stack) {
    onSyscall_d.emit(state, pc, syscallId, stack);
}
