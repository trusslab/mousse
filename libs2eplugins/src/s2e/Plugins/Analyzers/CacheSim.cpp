...
//    assert(false && "Need to implement onConcreteDataMemoryAccess");
    ...
    m_enable = conf->getBool(getConfigKey() + ".enableWhenStart", true);
    ...
    s2e()->getCorePlugin()->onConcreteDataMemoryAccess.connect(
            sigc::mem_fun(*this, &CacheSim::onConcreteDataMemoryAccess));

    m_cacheLog.reserve(CACHESIM_LOG_SIZE);
}

void CacheSim::enable(bool en) {
    m_enable = en;
}
...
void CacheSim::onConcreteDataMemoryAccess(S2EExecutionState *state, uint64_t virtualAddress,
                                          uint64_t value, uint8_t size, unsigned flags) {
    if (!m_enable)
        return;
        //      getDebugStream() << "acc pc=" << std::hex << state->regs()->getPc() << " ha=" << constAddress << '\n';

    onMemoryAccess(state, virtualAddress, size, flags & MEM_TRACE_FLAG_WRITE, flags & MEM_TRACE_FLAG_IO, false);
}
