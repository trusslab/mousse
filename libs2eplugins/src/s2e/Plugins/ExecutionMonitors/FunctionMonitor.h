...
public:
    ...
    CallSignal *getCallSignal(S2EExecutionState *state, uint64_t eip, uint64_t cr3 = -1);
    ...
    void printMap(S2EExecutionState *state);
...
class FunctionMonitorState : public PluginState {
    ...
    /* Get a signal that is emitted on function calls. Passing eip = -1 means
       any function, and cr3 = -1 means any cr3 */
    FunctionMonitor::CallSignal *getCallSignal(uint64_t eip, uint64_t cr3 = -1);
