...
#else
    assert(false && "FunctionMonitorState::registerReturnSignal failed\n");
	bool ok = false;
#endif
    if (!ok) {
#if defined(TARGET_I386)
        ...
        m_plugin->getWarningsStream(state) << "Function call with symbolic SP!\n"
                                           ...
                                           << " CP15 c2=" << hexval(state->regs()->getPageDir()) << '\n';
#endif
        return;
    }
...
void FunctionMonitorState::slotRet(S2EExecutionState *state, uint64_t pc, bool emitSignal) {
...
#elif defined(TARGET_ARM)
    target_ulong cr3 = state->regs()->read<target_ulong>(CPU_OFFSET(cp15.c2_base0));
    ...
    if (!ok) {
        ...
        m_plugin->getWarningsStream(state) << "Function return with symbolic SP!" << '\n'
                                           << "  PC=" << hexval(pc) << " CP15 c2=" << hexval(cr3) << '\n';
        return;
    }
#else
	assert(false && "FunctionMonitorState::slotRet failed\n");
#endif
