...
#ifndef MOUSSE_COVERAGE_PLUGIN_H
#define MOUSSE_COVERAGE_PLUGIN_H
...
class MousseCoverage : public Plugin {
    S2E_PLUGIN
public:
    ///
    ...
    MousseCoverage(S2E *s2e) : Plugin(s2e) {
    }
    ...
    void sendCoverageToServer(S2EExecutionState *state, int stateId);
...
private:
    DistributedExecution *m_distributedExecution;
    ...
    void onTimer();
    void onStateKill(S2EExecutionState *state, int stateId);
    void onTranslateBlockComplete(S2EExecutionState *state, TranslationBlock *tb,
                                  uint64_t last_pc);
...
#endif // MOUSSE_COVERAGE_PLUGIN_H
