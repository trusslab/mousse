...
void BaseInstructions::initialize() {
    ...
    m_useFuzzer = cfg->getBool(top + ".useFuzzer", false);
    m_useInitialInputs = cfg->getBool(top + ".useInitialInputs", false);
    m_stopBeforeTesting = cfg->getBool(top + ".stopBeforeTesting", false);
...
void BaseInstructions::onTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb,
	...
	assert(false && "onTranslateBlockStart failed\n");
#endif
}

void BaseInstructions::allowCurrentPid(S2EExecutionState *state) {
	assert(false && "allowCurrentPid failed\n");
}
...
void BaseInstructions::makeSymbolic(S2EExecutionState *state, uintptr_t address, unsigned size,
   ...
   if(m_useFuzzer) {
    ...
    fprintf(stderr, "Random Inputs = %s\n", (char *)address); 
   } else if(m_stopBeforeTesting) {
        // Kill the current state
        getWarningsStream(state) << "Before testing API: Killing state " << state->getID() << '\n';
        std::ostringstream os;
        os << "State was terminated before API testing\n";
        s2e()->getExecutor()->terminateStateEarly(*state, os.str());
    } else if (m_useInitialInputs) {
        fprintf(stderr, "Initial Inputs = %s\n", (char *)address); 
    } else {
        uint8_t *alternativeConcreteData = NULL;
        onBeforeMakeSymbolic.emit(state, address, size, nameStr, alternativeConcreteData);
        ...
        getWarningsStream(state) << "thread_id = " << hexval((unsigned long)pthread_self()) << ", Inserted symbolic data @" << hexval(address) << " of size " << hexval(size) << ": "
...
void BaseInstructions::isSymbolic(S2EExecutionState *state) {
    ...
    if (result) {
        onIsSymbolic.emit(state, address, size);
    }

    getInfoStream(state) << "Testing whether data at " << hexval(address) << " and size " << size
                         << " is symbolic: " << (result ? " true" : " false") << '\n';
#ifdef TARGET_ARM
    // FIXME since we hit the assertion when calling writeSymbolicRegion we disable the return for now
...
void BaseInstructions::sleep(S2EExecutionState *state) {
	assert(false && "BaseInstructions::sleep failed\n");
}
...
void BaseInstructions::invokePlugin(S2EExecutionState *state) {
	assert(false && "BaseInstructions::invokePlugin failed\n");
}
...
#else
	assert(false && "BaseInstructions::assume failed\n");
#endif
...
void BaseInstructions::assumeRange(S2EExecutionState *state) {
    ...
    lower = state->regs()->read(CPU_OFFSET(regs[1]), klee::Expr::Int32);
    upper = state->regs()->read(CPU_OFFSET(regs[2]), klee::Expr::Int32);
#else
	assert(false && "BaseInstructions::assumeRange failed\n");
#endif
...
void BaseInstructions::writeBuffer(S2EExecutionState *state) {
	assert(false && "BaseInstructions::writeBuffer failed\n");
}

void BaseInstructions::getRange(S2EExecutionState *state) {
	assert(false && "BaseInstructions::getRange failed\n");
}

void BaseInstructions::getConstraintsCountForExpression(S2EExecutionState *state) {
	assert(false && "BaseInstructions::getConstraintsCountForExpression failed\n");
}
...
void BaseInstructions::forkCount(S2EExecutionState *state) {
	assert(false && "BaseInstructions::forkCount failed\n");
}
...
void BaseInstructions::handleBuiltInOps(S2EExecutionState *state, uint64_t opcode) {
    ...
    switch ((opcode >> 8) & 0xFF) {
...
#ifdef TARGET_ARM
            printMessage(state, opcode >> 8 & 0x1);
#elif defined(TARGET_I386)
        ...
        case BASE_S2E_STATE_COUNT: { /* Get number of active states */
		    assert(false && "BASE_S2E_STATE_COUNT failed\n");
            break;
        }

        case BASE_S2E_INSTANCE_COUNT: { /* Get number of active S2E instances */
		    assert(false && "BASE_S2E_INSTANCE_COUNT failed\n");
            break;
        }
        ...
        case BASE_S2E_CLEAR_TEMPS: { /* s2e_invoke_plugin_concrete */
             ...
             */
		    assert(false && "BASE_S2E_CLEAR_TEMPS failed\n");
        } break;
