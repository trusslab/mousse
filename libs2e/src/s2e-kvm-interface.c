...
#endif
extern void *g_s2e;
/* global env should be created per thread */
...
void printSigset(const char *prefix, const sigset_t *sigset)
{
    ...
    if (cnt == 0)
        fprintf(stderr, "%s<empty signal set>\n", prefix);
}
#endif
...
#elif defined(TARGET_ARM)
    g_cpu_env = env = cpu_arm_init("qemu32-s2e");
#else
#error unknown architecture
    ...
    s2e_init();
/* moved from cpu_arm_init. g_s2e is only available after S2E is initialized */
#ifdef TARGET_ARM
    cpu_state_reset(env);
#endif
...
int s2e_kvm_vm_set_user_memory_region(int vm_fd, struct kvm_userspace_memory_region *region) {
    ...
    tlb_flush(env, 1);
#ifdef CONFIG_USER_KVM
    kvm_set_user_phys_mem(region->guest_phys_addr, region->memory_size);
#else
...
int s2e_kvm_vm_register_fixed_region(int vm_fd, struct kvm_fixed_region *region) {
    ...
    s2e_register_ram2((const char*)ram_name, region->host_address, region->size, region->flags & KVM_MEM_SHARED_CONCRETE);
#else
...
#ifdef CONFIG_USER_KVM
int s2e_kvm_vm_update_pageDesc(int vm_fd, struct kvm_user_update_page *page) {
	if(page->Invalidate) {
		kvm_user_tb_invalidate_phys_range(page->start_address, page->sizeOrend, page->flags);	
	} else {
		kvm_user_page_set_flags(page->start_address, page->sizeOrend, page->flags);
	}	
	return 0;
}
#endif
...
#ifdef DEBUG_CPU_EXIT_SIGNAL
	printSigset("\t\t",&s_s2e_kvm_sigmask.sigset);
#endif
    return 0;
}
...
static void block_signals(void) {
	...
	sigset_t set;
	sigemptyset(&set);
        if(!pthread_sigmask(SIG_BLOCK, NULL, &set))
		printSigset("\t\t",&set);
#endif
...
#endif
	env = (CPUArchState *)opaque;
//    fprintf(stderr, "%s [1], First vcpu thread ID = %lx\n",__FUNCTION__, (unsigned long)pthread_self());
    while (1) {
...
#else
#ifdef TARGET_I386
	process_cpu_x86_exec(env);
#elif defined(TARGET_ARM)
	process_cpu_arm_exec(env);
#endif
#endif
#ifdef SE_KVM_DEBUG_IRQ
...
int s2e_kvm_vcpu_run(int vcpu_fd) {
    int ret = 0;
    ++g_stats.kvm_runs;
/* Coroutine is not working for arm because of the lack of setcontext and getcontext implementations in boinic. */
    ...
    qemu_coroutine_enter(s_kvm_cpu_coroutine, env);
/* FIXME Coroutine is not working for arm because of the lack of setcontext and getcontext implementations in boinic. -- Update: ignore these two lines now. */
    if (s_s2e_exiting) {
        pthread_mutex_unlock(&s_cpu_lock);
        g_kvm_vcpu_buffer->exit_reason = KVM_EXIT_INTR;
        kill(getpid(), SIGTERM);
...
#ifdef SE_KVM_DEBUG_RUN
    if (!g_handling_kvm_cb) {
        printf("%s [4] riw=%d rii=%d exit_reason=%#x cr8=%#x\n", __FUNCTION__, g_kvm_vcpu_buffer->request_interrupt_window,
...
int s2e_kvm_vcpu_interrupt(int vcpu_fd, struct kvm_interrupt *interrupt) {
	assert(false && "s2e_kvm_vcpu_interrupt failed\n");
}

int s2e_kvm_vcpu_nmi(int vcpu_fd) {
	assert(false && "s2e_kvm_vcpu_nmi failed\n");
}
