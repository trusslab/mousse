...
DATA_TYPE glue(glue(io_read, SUFFIX), MMUSUFFIX)(ENV_PARAM target_phys_addr_t physaddr, target_ulong addr,
    ...
    if (is_notdirty_ops(ops)) {
        ...
        if (likely(_se_check_concrete(e->objectState, addr & ~SE_RAM_OBJECT_MASK, DATA_SIZE))) {
             ...
             * the helper had done.
             */
            return glue(glue(ld, USUFFIX), _raw)((uint8_t *) (addr + (e->se_addend)));
        } else {
            g_sqi.exec.switch_to_symbolic(retaddr);
        }
    }
...
#endif
redo:
    tlb_entry = &tlb_table[mmu_idx][index];
    ...
    if (likely((addr & TARGET_PAGE_MASK) == (tlb_addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK)))) {
        if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
            ...
            retaddr = GETPC();
#endif
            ioaddr = iotlb[mmu_idx][index];
#ifdef CONFIG_SYMBEX
            se_tlb_current = tlb_entry;
#endif
...
#endif
//check if it's writing to symbolic memory. If so, change the memory status
    g_sqi.mem.verify_write_ram_concrete((uint64_t)addr, DATA_SIZE);

    SE_SET_MEM_IO_VADDR(env, addr, 0);
    env->mem_io_pc = (uintptr_t) retaddr;
#if SHIFT <= 2
    ops->write(physaddr, val, 1 << SHIFT);
//    g_sqi.mem.verify_write_ram_concrete((uint64_t)addr, 1 << SHIFT);
#else
#ifdef TARGET_WORDS_BIGENDIAN
