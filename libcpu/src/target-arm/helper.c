...
/// Copyright (C) 2020, TrussLab@University of California
...
//    	target_set_brk((ts->info)->brk);
    	target_set_brk(ts->heap_base);
    g_sqi.events.on_load_image_end((ImageInfo*)(ts->info));
}
#endif
static void cpu_reset_model_id(CPUARMState *env, uint32_t id) {
    ...
    switch (id) {
        ...
        case ARM_CPUID_SA1110:
            ...
            env->cp15.c1_sys = 0x00000070;
            break;
/* copied form case ARM_CPUID_ANY */
        case ARM_CPUID_QEMU32_S2E:
            set_feature(env, ARM_FEATURE_V7);
            set_feature(env, ARM_FEATURE_VFP4);
            set_feature(env, ARM_FEATURE_VFP_FP16);
            set_feature(env, ARM_FEATURE_NEON);
...
void *rom_ptr(target_phys_addr_t addr) {
	assert(false && "rom_ptr not implemented\n");
}
void cpu_state_reset(CPUARMState *env) {
    ...
    env->tlb_table_addr = (uint32_t)tlb_table;
...
CPUARMState *cpu_arm_init(const char *cpu_model) {
    ...
    env->cp15.c0_cpuid = id;
/* Do this again after the s2e is initialized */
    cpu_state_reset(env);
    if (arm_feature(env, ARM_FEATURE_NEON)) {
...
static const struct arm_cpu_t arm_cpu_names[] = {{ARM_CPUID_ARM926, "arm926"},
                                                 ...
                                                 {ARM_CPUID_QEMU32_S2E, "qemu32-s2e"},
                                                 {ARM_CPUID_ANY, "any"},
                                                 {0, NULL}};
...
static void do_interrupt_v7m(CPUARMState *env) {
    ...
    switch (env->exception_index) {
        case EXCP_UDEF:
	    assert(false && "EXCP_UDEF not supported\n");
            return;
        case EXCP_SWI:
	    assert(false && "EXCP_SWI not supported\n");
            return;
        case EXCP_PREFETCH_ABORT:
        case EXCP_DATA_ABORT:
	    assert(false && "EXCP_DATA_ABORT not supported\n");
            return;
        case EXCP_BKPT:
            if (semihosting_enabled) {
			assert(false && "semihosting not supportted\n");
            }
	    assert(false && "EXCP_BKPT not supported\n");
            return;
        case EXCP_IRQ:
...
#if defined(CONFIG_USER_KVM) && defined(CONFIG_SYMBEX)
...
{
	assert(false && "se_do_interrupt_all should not be called here\n");
}
#else
/* Handle a CPU exception.  */
void do_interrupt(CPUARMState *env) {
	assert(false && "system mode do_interrupt for arm is not supported\n");
    uint32_t addr;
    uint32_t mask;
    int new_mode;
    uint32_t offset;
...
int cpu_arm_handle_mmu_fault(CPUARMState *env, target_ulong address, int access_type, int mmu_idx) {
    if(address == 0xfffffc00)
            assert(0 && "cpu_arm_handle_mmu_fault\n");
    uint32_t phys_addr;
...
//copied from https://lists.gnu.org/archive/html/qemu-devel/2013-12/msg00196.html
/* ARMv8 VMAXNM/VMINNM */
...
int arm_rmode_to_sf(int rmode)
{
    ...
    case FPROUNDING_ODD:
        ...
        fprintf(stderr, "arm: unimplemented rounding mode: %d\n",
                      rmode);
