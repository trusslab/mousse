...
//adding DisasCompare from upstream QEMU
...
/* moved from target-arm/cpu.h to resolve some conflicts */
...
//    // Ignore thumb mode
//    if (ARM_TBFLAG_THUMB(s->tb->flags) & 1) {
//        return;
//    }
...
//new condition code handling pulled from upstream QEMU
//commit 6c2c63d3a02c79e9035ca0370cc549d0f938a4dd
...
void arm_test_cc(DisasCompare *cmp, int cc)
{
    ...
    case 15: /* always */
        ...
        assert(false && "Need to update TCG_COND_XX definition");
...
#ifdef CONFIG_SYMBEX
    gen_eob_event(s, 1, addr & ~1);
#endif
}
/* Set PC and Thumb state from var.  var is marked as dead.  */
static inline void gen_bx_no_eob(DisasContext *s, TCGv var)
{
    s->is_jmp = DISAS_UPDATE;
...
static inline void gen_bx(DisasContext *s, TCGv var)
{
    ...
    store_cpu_field(var, thumb);
#ifdef CONFIG_SYMBEX
    // target_pc not determined during translation
    gen_eob_event(s, 0, 0);
#endif
}
...
static inline void store_reg_from_load_no_eob(CPUARMState *env, DisasContext *s,
                                int reg, TCGv var)
{
...
static int disas_vfp_v8_insn(CPUARMState *env, DisasContext *s, uint32_t insn)
{
    ...
    if (!arm_feature(env, ARM_FEATURE_V8)) {
        fprintf(stderr, "ARM_FEATURE_V8 not supported\n");
        return 1;
    }
...
void instr_gen_flags_update(void *context) {
	assert(0 && "instr_gen_flags_update: to be implemented\n");
}

void instr_gen_pc_update(void *context, target_ulong pc, target_ulong cs_base);
void instr_gen_pc_update(void *context, target_ulong pc, target_ulong cs_base) {
	if(cs_base)
		assert(0 && "instr_gen_pc_update, please check cs_base\n");
    TCGv cpu_tmp0;
    cpu_tmp0 = new_tmp();
    tcg_gen_movi_tl(cpu_tmp0, pc - cs_base);
    tcg_gen_st_tl(cpu_tmp0, cpu_env, offsetof(CPUArchState, regs[15]));
...
static void disas_arm_insn(CPUARMState * env, DisasContext *s)
{
    ...
    if ((insn >> 24) == 255) { /* s2e_op */
            ...
            if (likely(*g_sqi.mode.allow_custom_instructions)) {
                instr_gen_pc_update(s, pc_start, 0);
                g_sqi.events.tcg_emit_custom_instruction((uint64_t) insn);
            } else {
                goto illegal_op;
            }
		return;
//s2e target-i386: end
    ...
    if (cond == 0xf){
        ...
        if ((insn & 0x0ffffdff) == 0x01010000) {
            ...
            assert(0 && "[2.921]: NOT SUPPORTED\n");
        } else if ((insn & 0x0f000a00) == 0x0e000800) {
            assert(0 && "[2.922]: NOT SUPPORTED\n");
            return;
...
/* A patch from https://ipads.se.sjtu.edu.cn:1312/qiuzhe/qemu-official/commit/2359bf80c1c4e8ed1e7ddb03661fec6bace82a87 */
        ...
        do_ldst:
            ...
            // ldr pc, [ip, #imm]!
            // Must instrument after branch target address being written back to ip
            if ((insn & 0x0f7ff000) == 0x053cf000) {
                SET_TB_TYPE(TB_JMP_PLT);
#ifdef CONFIG_SYMBEX
                if (unlikely(*g_sqi.events.on_translate_jump_start_signals_count && s->instrument)) {
                    g_sqi.events.on_translate_jump_start(s, s->tb, pc_start, JT_PLT);
                }
#endif
            }
            if (insn & (1 << 20)) {
                    ...
                    if (insn & (1 << i)) {
                        if (insn & (1 << 20)) {
                            ...
                            if (user) {
                                ...
                                store_reg_from_load_no_eob(env, s, i, tmp);
                            }
                        } else {
                            /* store */
                            if (i == 15) {
...
//#ifdef CONFIG_USER_ONLY
        ...
        if (generate_pc_recovery_info && !search_pc) {
            ...
            p->cc_op = (dc->condexec_cond << 4) | (dc->condexec_mask >> 1);
            p->opc = cur_opc;
            ++p;
            ++tb->precise_entries;
        }
        ...
        /* Generate precise PC recovery information */
        if (generate_pc_recovery_info && !search_pc) {
            (p - 1)->guest_inst_size = dc->pc - pc_ptr;
        }
#endif
...
{
    target_ulong newpc = pc;
    env->regs[15] = newpc;
    env->condexec_bits = cc_op;
}
#endif
