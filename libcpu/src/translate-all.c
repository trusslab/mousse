...
void cpu_gen_init(void) {
    ...
    tcg_ctx.env_offset_eip = offsetof(CPUArchState, regs[15]);
    tcg_ctx.env_sizeof_eip = sizeof(env->regs[15]);
#endif
...
#if 0
//#if 1
    libcpu_log("RESTORE: searched_pc=%#"PRIx32" tc_ptr=%#"PRIx32" tc_ptr_max=%#"PRIx32" icount=%d cur_pc=%#x\n",
             searched_pc, (uintptr_t)tb->tc_ptr, (uintptr_t)tb->tc_ptr + tb->tc_size, tb->icount, env->regs[15]);
#endif
    if (!g_sqi.exec.is_running_concrete()) {
#ifdef ENABLE_PRECISE_EXCEPTION_DEBUGGING
        assert(false && "PRECISE_EXCEPTION_DEBUGGING is not supported for arm\n");
#endif
#ifdef ENABLE_PRECISE_EXCEPTION_DEBUGGING_COMPARE
