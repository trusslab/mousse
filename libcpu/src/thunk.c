...
const argtype *deep_check(void *dst, const void *src,
                             const argtype *type_ptr, int access)
{
    int type;
    type = *type_ptr++;
    switch(type) {
...
//any address?
    ...
    case TYPE_PTRVOID:
        {
             if (access == VERIFY_READ) {
                 access_ok(VERIFY_READ, (abi_ulong)src, 4);
             } else {
                 access_ok(VERIFY_WRITE, (abi_ulong)src, 4);
             }
        }
         break;
    case TYPE_ARRAY:
    ...
    case TYPE_STRUCT:
        {
            ...
            if (se->convert[0] != NULL) {
                ...
                if(access == VERIFY_READ)
                    (*se->convert[0])(NULL, src);
                else
                    (*se->convert[1])(NULL, src);
            } else {
                /* standard struct conversion */
                field_types = se->field_types;
                src_offsets = se->field_offsets[1];
                for(i = 0;i < se->nb_fields; i++) {
                    field_types = deep_check(NULL,
                                                src + src_offsets[i],
                                                field_types, access);
                }
            }
