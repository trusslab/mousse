              ...
              abi_long arg4, abi_long arg5, abi_long arg6)
{
    ...
    gemu_log("%llx pid = %d ", (unsigned long long)pthread_self(), getpid() );
    ...
    for(i=0;i<nsyscalls;i++)
        if( scnames[i].nr == num ) {
            if( scnames[i].result != NULL ) {
                ...
                if (errstr) {
                    gemu_log("syscall: %d return = -1 errno=" TARGET_ABI_FMT_ld " (%s)\n",
                             num, -ret, errstr);
                } else {
                    gemu_log("syscall:%d return = " TARGET_ABI_FMT_ld "\n", num, ret);
                }
            }
            break;
        }
