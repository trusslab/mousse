...
{
	assert(false && "tb_invalidate_phys_page: to be fixed\n");
    TranslationBlock *tb;
    PageDesc *p;
    int n;
...
void user_tb_invalidate_phys_range(target_ulong start, target_ulong end,
                              int is_cpu_write_access)
{
    tb_page_addr_t start_new, end_new;
    start_new = qemu_ram_addr_from_host_nofail(g2h(start));
    end_new = start_new + (end - start);
    while (start_new < end_new) {
        tb_invalidate_phys_page_range(start_new, end_new, is_cpu_write_access);
        start_new &= TARGET_PAGE_MASK;
        start_new += TARGET_PAGE_SIZE;
    }
}
#endif
