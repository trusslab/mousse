...
target_phys_addr_t iotlb_ramaddr[NB_MMU_MODES][CPU_TLB_SIZE];
#endif
...
void tlb_flush(CPUArchState *env, int flush_global) {
    int i;
#ifdef CONFIG_SYMBEX
/* if s2e is not initilized, return */
    if (!get_s2e()) {
        return;
    }
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        for (i = 0; i < CPU_TLB_SIZE; i++) {
            tlb_table[mmu_idx][i] = s_cputlb_empty_entry;
        }
    }
...
void tlb_flush_page(CPUArchState *env, target_ulong addr) {
    ...
    if ((addr & tlb_flush_mask) == tlb_flush_addr) {
#if defined(DEBUG_TLB)
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        CPUTLBEntry *tlb_entry = &tlb_table[mmu_idx][i];
        if (addr == (tlb_entry->addr_read & (TARGET_PAGE_MASK | TLB_INVALID_MASK)) ||
            ...
            g_sqi.tlb.flush_tlb_cache_page(tlb_table[mmu_idx][i].objectState, mmu_idx, i);
#endif
...
void cpu_tlb_update_dirty(CPUArchState *env) {
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        for (i = 0; i < CPU_TLB_SIZE; i++)
            tlb_update_dirty(&tlb_table[mmu_idx][i]);
    }
}
...
int tlb_is_dirty(CPUArchState *env, target_ulong vaddr) {
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        ret &= tlb_get_dirty1(&tlb_table[mmu_idx][i], vaddr);
    }
...
static void tlb_add_large_page(CPUArchState *env, target_ulong vaddr, target_ulong size) {
    ...
    if (tlb_flush_addr == (target_ulong) -1) {
        tlb_flush_addr = vaddr & mask;
        tlb_flush_mask = mask;
        return;
    }
    ...
    mask &= tlb_flush_mask;
    while (((tlb_flush_addr ^ vaddr) & mask) != 0) {
        mask <<= 1;
    }
    tlb_flush_addr &= mask;
    tlb_flush_mask = mask;
...
void tlb_set_page(CPUArchState *env, target_ulong vaddr, target_phys_addr_t paddr, int prot, int mmu_idx,
    ...
    CPUTLBEntry *te;
    CPUWatchpoint *wp;
    target_phys_addr_t local_iotlb;

#if defined(DEBUG_TLB)
    fprintf(stderr, "%s [1]: vaddr=" TARGET_FMT_lx " paddr=0x" TARGET_FMT_plx " prot=%x mmu_idx=%d index=%u size = %x, thread_id = %llx\n",__FUNCTION__, vaddr, paddr, prot, mmu_idx, (vaddr >> TARGET_PAGE_BITS) & (CPU_TLB_SIZE - 1), size, (unsigned long long)pthread_self());
#endif
...
#if defined(DEBUG_TLB)
    printf("%s [2]: MemoryDesc sreg->ram_addr=0x%llx sreg->kvm.guest_phys_addr=0x%llx sreg->kvm.userspace_addr=0x%llx sreg->kvm.memory_size=0x%llx sreg->kvm.slot=%u\n",__FUNCTION__, sreg->ram_addr, sreg->kvm.guest_phys_addr,sreg->kvm.userspace_addr,sreg->kvm.memory_size,sreg->kvm.slot);
#endif
    address = vaddr;
    ...
    if (sreg) {
        ...
        local_iotlb = (sreg->ram_addr & TARGET_PAGE_MASK) + mem_desc_get_offset(sreg, paddr);
        local_iotlb |= phys_section_notdirty;
    } else {
#ifdef CONFIG_USER_KVM
    fprintf(stderr, "%s [1]: No MMIO in user kvm, vaddr=" TARGET_FMT_lx " paddr=0x" TARGET_FMT_plx " prot=%x mmu_idx=%d index=%u, thread_id = %llx\n",__FUNCTION__, vaddr, paddr, prot, mmu_idx, (vaddr >> TARGET_PAGE_BITS) & (CPU_TLB_SIZE - 1), (unsigned long long)pthread_self());
    fprintf(stderr, "This might due to symbolic address access in target program, need further investigation\n");
    s2e_kill_state("This might due to symbolic address access in target program, need further investigation\n");
    exit(-1);
#endif
    ...
    QTAILQ_FOREACH (wp, &env->watchpoints, entry) {
        if (vaddr == (wp->vaddr & TARGET_PAGE_MASK)) {
            ...
            if ((prot & PAGE_WRITE) || (wp->flags & BP_MEM_READ)) {
                local_iotlb = phys_section_watch + paddr;
                address |= TLB_MMIO;
                break;
            }
        }
    ...
    iotlb[mmu_idx][index] = local_iotlb - vaddr;
//   fprintf(stderr, "%s [2.3]: env->iotlb[mmu_idx][index] = %llx vaddr = %x\n",__FUNCTION__, env->iotlb[mmu_idx][index], vaddr);
...
#ifdef CONFIG_USER_KVM
	assert(false && "Fault, No MMIO in user kvm!\n");
#endif
            // MMIO
...
#endif
#if defined(DEBUG_TLB)
    //printf("%s [4]:CPUTLBEntry te->addend = %lx te->se_addend = %lx, te->addr_code = %lx te->addr_read = %lx te->addr_write = %lx env->iotlb[mmu_idx][index] = %lx\n", __FUNCTION__, te->addend, te->se_addend, te->addr_code, te->addr_read, te->addr_write, env->iotlb[mmu_idx][index]);
#endif
}
