...
#ifdef TARGET_I386
#include "target-i386/target_signal.h"
#elif defined(TARGET_ARM)
#include "target-arm/target_signal.h"
#endif
...
static void LIBCPU_NORETURN force_sig(int target_sig)
{
...
static inline int handle_cpu_signal(uintptr_t pc, unsigned long address,
                                    ...
                                    void *puc)
{
    fprintf(stderr, "%s [1]: qemu: SIGSEGV pc=0x%08x address=%08lx w=%d oldset=0x%08lx thread_id = %lx\n",
                __FUNCTION__, pc, address, is_write, *(unsigned long *)old_set ,(unsigned long)pthread_self());
	assert(0 && "should not come to here\n");
}
                                ...
                                void *puc)
{
    ...
    g_sqi.events.on_receive_signal(host_signum);
    ...
    /* the CPU emulator use SIGUSR2 signal to exit,
       we forward to it this signal. What if the signal is for the guest process? FIXME */
    if (host_signum == SIGUSR2) {
    ...
    fprintf(stderr, "pid = %d %s: thread = %lx env = %p got host_signal %d\n", getpid(),__FUNCTION__,(unsigned long)pthread_self(), env, host_signum);
#endif
    //FIXME: what happened?
    if(host_signum == 35)
    {
...
void process_pending_signals(CPUArchState *cpu_env)
{
 ...
 handle_signal:
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "thread = %lx process signal %d\n", (unsigned long)pthread_self(), sig);
#endif
    /* dequeue signal */
    q = k->first;
    k->first = q->next;
...
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [1]: process signal %d\n",__FUNCTION__,sig);
#endif
    if (handler == TARGET_SIG_DFL) {
        ...
        if (sig == TARGET_SIGTSTP || sig == TARGET_SIGTTIN || sig == TARGET_SIGTTOU) {
            kill(getpid(),SIGSTOP);
                   ...
                   sig != TARGET_SIGCONT) {
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [2]: process signal %d\n",__FUNCTION__,sig);
#endif
            force_sig(sig);
        }
...
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [2.1]: process signal %d\n",__FUNCTION__,sig);
#endif
...
#endif
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [3]: process signal %d\n",__FUNCTION__,sig);
#endif
        /* prepare the stack frame of the virtual CPU */
        if (sa->sa_flags & TARGET_SA_SIGINFO) {
            setup_rt_frame(sig, sa, &q->info, &target_old_set, cpu_env);
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [3.1]: process signal %d\n",__FUNCTION__,sig);
#endif
        }
        else {
            setup_frame(sig, sa, &target_old_set, cpu_env);
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [3.2]: process signal %d\n",__FUNCTION__,sig);
#endif
        }
	if (sa->sa_flags & TARGET_SA_RESETHAND) {
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [3.3]: process signal %d\n",__FUNCTION__,sig);
#endif
            sa->_sa_handler = TARGET_SIG_DFL;
        }
    }
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [4]: process signal %d\n",__FUNCTION__,sig);
#endif
    if (q != &k->info)
        free_sigqueue(cpu_env, q);
#ifdef DEBUG_SIGNAL
    fprintf(stderr, "%s [5]: process signal %d\n",__FUNCTION__,sig);
#endif
}
