...
#endif
            INSTR_AFTER_MEMORY_ACCESS(addr, 0, MEM_TRACE_FLAG_IO, retaddr);
            }
		return (void *) (addr + tlb_entry->se_addend);
    } else {
#ifndef SYMBEX_LLVM_LIB
        retaddr = GETPC();
#endif
        tlb_fill(env, addr, object_index << SE_RAM_OBJECT_BITS, 0, mmu_idx, retaddr);
        goto redo;
    }
} 
#endif
