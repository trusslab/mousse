...
#endif
            CPUTLBEntry *entry = &tlb_table[coords.first][coords.second];
...
void S2EExecutionStateTlb::flushTlbCachePage(klee::ObjectState *objectState, int mmu_idx, int index) {
    ...
    tlb_table[mmu_idx][index].objectState = 0;
    tlb_table[mmu_idx][index].se_addend = 0;
...
/* This could happen: arm arch && clone syscall exits && m_tlbMap is flushed when creating a new thread_env */
#if defined(CONFIG_USER_KVM) && defined(TARGET_ARM)
    if(tlbIt == m_tlbMap.end())
		return;
#else
...
void S2EExecutionStateTlb::updateTlbEntryConcreteStatus(CPUArchState *env, unsigned mmu_idx, unsigned index,
    ...
    CPUTLBEntry *te = &tlb_table[mmu_idx][index];
    ...
    CPUTLBRAMEntry *re = &se_ram_tlb[ram_tlb_index];
    re->host_page = 0;
    re->addend = 0;
    re->object_state = 0;
#endif
...
void S2EExecutionStateTlb::clearRamTlb() {
    ...
    for (unsigned i = 0; i < CPU_TLB_SIZE; i++) {
        se_ram_tlb[i] = NULLCPUTLBRAMEntry;
    }
}
#endif
...
void S2EExecutionStateTlb::clearTlbOwnership() {
    ...
    for (unsigned i = 0; i < CPU_TLB_SIZE; i++) {
        for (unsigned mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
            tlb_table[mmu_idx][i].addr_write |= TLB_NOT_OURS;
        }
    }
}
...
/* The content in hostAddr is copied to the ConcreteStore buffer in symbolic mode. So we use the address of the ConcreteStore buffer as the host address */
...
bool S2EExecutionStateTlb::audit() {
    ...
    foreach2 (tlbIt, m_tlbMap.begin(), m_tlbMap.end()) {
        ...
        foreach2 (vit, vec.begin(), vec.end()) {
            ...
            CPUTLBEntry *entry = &tlb_table[mmu_idx][index];
            if(entry->objectState != os) {
                fprintf(stderr, "%s: entry->objectState = %p mmu_idx = %d index = %d os = %p\n",__FUNCTION__, (void *)entry->objectState, mmu_idx, index, (void *)os);
            }
    ...
    for (unsigned i = 0; i < CPU_TLB_SIZE; i++) {
        for (unsigned mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
            CPUTLBEntry *entry = &tlb_table[mmu_idx][i];
            ...
            if(it == m_tlbMap.end()) {
                fprintf(stderr, "%s: entry->objectState = %p mmu_idx = %d i = %d m_tlbMap.size() = %d\n",__FUNCTION__, (void *)entry->objectState, mmu_idx, i, m_tlbMap.size());
            }
