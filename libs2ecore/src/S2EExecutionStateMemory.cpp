...
/***/
void S2EExecutionStateMemory::changeSymbolicRamInternal(ObjectPair op, uint64_t object_offset, uint64_t size) {
    ...
    for (uint64_t i = 0; i < size; ++i) {
            wos->mark8(object_offset + i);
    }
    bool newAllConcrete = wos->isAllConcrete();
//    if(oldAllConcrete != newAllConcrete)
...
void S2EExecutionStateMemory::transferRamInternal(ObjectPair op, uint64_t object_offset, uint8_t *buf, uint64_t size,
    ...
    if (op.first->isSharedConcrete) {
        ...
        if (write) {
/* if this is called from access_ok, dest = source may cause undefined behavior */
            if((concreteStore + object_offset) != buf)
...
void S2EExecutionStateMemory::changeSymbolicRam(struct CPUTLBRAMEntry *te, uint64_t hostAddress, uint64_t size) {
    assert(*m_active);
    ...
    if (!(page_offset + size <= SE_RAM_OBJECT_SIZE)) {
        ...
        changeSymbolicRam(te, hostAddress, size1);
        changeSymbolicRam(te, hostAddress + size1, size - size1);
        return;
    }
    ...
    if (osSize == op.first->size) {
        ...
        changeSymbolicRamInternal(op, page_offset, size);
        return;
    }
    ...
    while (size > 0) {
        ...
        changeSymbolicRamInternal(op, page_offset, transferSize);
        size -= transferSize;
...
void S2EExecutionStateMemory::writeSymbolicRegionSilentInternal(ObjectPair op, uint64_t object_offset, uint64_t size) 
{
     uint32_t tmp_buf;
//     ObjectState *wos = m_addressSpace->getWriteable(op.first, op.second);
     bool AllConcrete = op.second->isAllConcrete();
     if(AllConcrete) {
            return;
     }
     int i;
     for(i = 0; i < size; i++) {
          if (!op.second->isConcrete(object_offset + i, 1)) {
                fprintf(stderr, "pid = %d Mousse: before multiConcretize\n", getpid());
                tmp_buf = m_concretizer->multiConcretize(op.second->read8(object_offset + i), "Mousse: Write evaluatedValue for symbolic Region with concretization");
                fprintf(stderr, "pid = %d Mousse: after multiConcretize\n", getpid());
                const_cast<ObjectState *>(op.second)->writeEvaluatedValue8(object_offset + i, (uint8_t)tmp_buf);
                fprintf(stderr, "pid = %d Mousse: Write 1 bytes evaluatedValue = %x for symbolic Region with concretization at address = %p, offset = %d\n", getpid(),tmp_buf, (void *)((char *)op.first->address + object_offset ), i);
            }
    }
//     for(i = 0; i + 4 <= size; i += 4) {
//          if (!op.second->isConcrete(object_offset + i, 4)) {
//                tmp_buf = m_concretizer->multiConcretize(op.second->read32(object_offset + i), "Mousse: Write evaluatedValue for symbolic Region with concretization");
//                const_cast<ObjectState *>(op.second)->writeEvaluatedValue32(object_offset + i, tmp_buf);
//            }
//    }
//    
//     for(; i < size; i++) {
...
void S2EExecutionStateMemory::writeSymbolicRegionSilent(struct CPUTLBRAMEntry *te, uint64_t hostAddress, uint64_t size) {  
    assert(*m_active);
    uint64_t page_offset = hostAddress & ~SE_RAM_OBJECT_MASK;
    if (!(page_offset + size <= SE_RAM_OBJECT_SIZE)) {
        /* Access spans multiple MemoryObject's */
        uint64_t size1 = SE_RAM_OBJECT_SIZE - page_offset;
        writeSymbolicRegionSilent(te, hostAddress, size1);
        writeSymbolicRegionSilent(te, hostAddress + size1, size - size1);
        return;
    }
    ...
    if (osSize == op.first->size) {
        ...
        writeSymbolicRegionSilentInternal(op, page_offset, size);
        return;
    }
    ...
    while (size > 0) {
        ...
        uint64_t transferSize = op.first->size < size ? op.first->size : size;
        writeSymbolicRegionSilentInternal(op, page_offset, transferSize);
        size -= transferSize;
        // Only the first access can access the middle of an object
        page_offset = 0;
        page_addr += transferSize;
