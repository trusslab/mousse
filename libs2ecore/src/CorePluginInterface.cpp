...
void s2e_on_other_make_concolic(uint64_t addr, unsigned size, const char *name) {
    try {
        g_s2e->getCorePlugin()->onOtherMakeConcolic.emit(g_s2e_state, addr, size, std::string(name));
...
void s2e_tcg_execution_handler(void *signal, uint64_t pc) {
    if (pc > 0xffffffff) {
//        fprintf(stderr, "%s Invalid pc=%llx. It is probably a bug and we ignore the call for now.\n", __FUNCTION__, pc);
        return;
    }
...
#elif defined(TARGET_ARM)
        assert(false && "s2e_tcg_instrument_code failed\n");
        // XXX haven't checked if it is correct
#endif
#endif
    }
...
void s2e_on_concretize_syscall_args(int is_reg, uint64_t addr, uint64_t size) {
    try {
        g_s2e->getCorePlugin()->onConcretizeSyscallArgs.emit(g_s2e_state, is_reg, addr, size);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_do_syscall_start(uint64_t syscall_id, SyscallArguments *args) {
    try {
        g_s2e->getCorePlugin()->onDoSyscallStart.emit(g_s2e_state, syscall_id, args);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_do_syscall_end(uint64_t syscall_id, uint64_t ret, SyscallArguments *args) {
    try {
        g_s2e->getCorePlugin()->onDoSyscallEnd.emit(g_s2e_state, syscall_id, ret, args);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_load_image_end(ImageInfo *info) {
    try {
        g_s2e->getCorePlugin()->onLoadImageEnd.emit(g_s2e_state, info);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_receive_signal(uint32_t signal) {
    try {
        g_s2e->getCorePlugin()->onReceiveSignal.emit(g_s2e_state, signal);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_thread_create(uint64_t child_stack, uint64_t ptid, uint64_t ctid) {
    try {
        g_s2e->getCorePlugin()->onThreadCreate.emit(g_s2e_state, child_stack, ptid, ctid);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}

void s2e_on_thread_exit(uint64_t tid) {
    try {
        g_s2e->getCorePlugin()->onThreadExit.emit(g_s2e_state, tid);
    } catch (s2e::CpuExitException &) {
        longjmp(env->jmp_env, 1);
    }
}
