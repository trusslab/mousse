...
redo:
    ...
    if (isWrite) {
        tlb_addr = tlb_table[mmu_idx][index].addr_write;
    } else {
        tlb_addr = tlb_table[mmu_idx][index].ADDR_READ;
    }
...
#endif
            addend = tlb_table[mmu_idx][index].addend;
...
void S2EExecutor::handle_ldst_kernel(Executor *executor, ExecutionState *state, klee::KInstruction *target,
    ...
    if (unlikely(tlb_table[mmu_idx][page_index].ADDR_READ != (addr & (TARGET_PAGE_MASK | (data_size - 1))))) {
        ...
        physaddr = addr + tlb_table[mmu_idx][page_index].addend;
