...
std::vector<ref<Expr>> S2EExecutionState::createConcolicArray(const std::string &name, unsigned size,
    ...
    if (concreteBuffer.size() == size) {
        if (ConcolicMode) {
            for (std::vector<unsigned char>::const_iterator i = concreteBuffer.begin(); i != concreteBuffer.end(); ++i)
...
bool S2EExecutionState::merge(const ExecutionState &_b) {
    ...
    for (; ai != ae && bi != be; ++ai, ++bi) {
        ...
        if (ai->second != bi->second && !ai->first->isValueIgnored &&
            ai->first != m_registers.getConcreteRegs() &&
...
uint64_t S2EExecutionState::solveSuggestedValueForConcolicVar(klee::ref<klee::Expr> expression){
...
uint64_t S2EExecutionState::multiConcretize(klee::ref<klee::Expr> expression, const std::string &reason) {
#ifdef CONFIG_SYMBEX_MP
//Never should be Constant
    assert(!dyn_cast<ConstantExpr>(expression));
    return g_s2e->getExecutor()->toMultiConstant(*this, expression, reason.c_str());
#else
     assert(0);
#endif
}
...
void S2EExecutionState::writeSyscallSymbolicRegions(uint64_t host_address, uint64_t size) {
#ifdef CONFIG_SYMBEX_MP
#if defined(SE_ENABLE_PHYSRAM_TLB)
    uintptr_t tlb_index = (host_address >> 12) & (CPU_TLB_SIZE - 1);
    CPUTLBRAMEntry *re = &se_ram_tlb[tlb_index];
#else
    CPUTLBRAMEntry *re =  NULL;
#endif
    //FIXME: change the name, it is not silent, it's concretizing the symbolic region it wants to read
    g_s2e_state->mem()->writeSymbolicRegionSilent(re, host_address, size);  
#endif
}
...
#ifdef TARGET_ARM
void s2e_get_brk_info(uint32_t *target_brk, int flag) {
	*target_brk = g_s2e_state->getTargetBrk(flag);
}

void s2e_set_brk_info(uint32_t target_brk, int flag) {
	g_s2e_state->setTargetBrk(target_brk, flag);
}
#else
void s2e_get_brk_info(uint64_t *target_brk, int flag) {
	*target_brk = g_s2e_state->getTargetBrk(flag);
}

void s2e_set_brk_info(uint64_t target_brk, int flag) {
	g_s2e_state->setTargetBrk(target_brk, flag);
}
#endif
int s2e_is_runnable() {
...
//syscall argument evaluation
void s2e_write_evaluatedValue_to_symbolicRegion(uint64_t host_address, uint64_t size) {
#ifdef CONFIG_SYMBEX_MP
    CPUTLBRAMEntry *re = s2e_get_ram_tlb_entry(host_address);
    g_s2e_state->mem()->writeSymbolicRegionSilent(re, host_address, size);  
#endif
}
void s2e_verify_write_ram_concrete(uint64_t host_address, uint64_t size) {
#ifdef CONFIG_SYMBEX_MP
    CPUTLBRAMEntry *re = s2e_get_ram_tlb_entry(host_address);
    if(re) {
        g_s2e_state->mem()->changeSymbolicRam(re, host_address, size);
    } else {
        assert(0 && "Invalid ram\n");
    }
#else
