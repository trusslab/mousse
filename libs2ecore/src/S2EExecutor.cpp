...
cl::opt<bool>
MousseConcretizeWOConstraints("mousse-concretize-wo-constraints",
            cl::desc("Don't add constraints when concretizing syscall arguments"),
            cl::init(false));
...
extern "C" {
    ...
    se_libcpu_update_klee_cpu_t se_libcpu_update_klee_cpu = &s2e::S2EExecutor::updateKleeCpuEnv;
...
thread_local MemoryObject *symbolicRegs = NULL;
thread_local MemoryObject *concreteRegs = NULL;
MemoryObject *main_symbolicRegs = NULL;
MemoryObject *main_concreteRegs = NULL;
/* External di//spatcher to convert longjmp's into C++ exceptions */
...
void S2EExecutor::handlerTraceInstruction(klee::Executor *executor, klee::ExecutionState *state,
	...
	assert(false && "handlerTraceInstruction");
#endif
}
...
void S2EExecutor::handleForkAndConcretize(Executor *executor, ExecutionState *state, klee::KInstruction *target,
    ...
    if (VerboseOnSymbolicAddress) {
        g_s2e->getWarningsStream(s2eState) << "thread = "<< hexval((unsigned long)pthread_self()) << " handleForkAndConcretize onSymbolicAddress at " << hexval(s2eState->regs()->getPc()) << " (reason "
...
#define __DEFINE_EXT_VARIABLE(name) llvm::sys::DynamicLibrary::AddSymbol(#name, (void *) &name);
    ...
    mousseConcretizeWOConstraints = MousseConcretizeWOConstraints;
...
////    __DEFINE_EXT_OBJECT_RO(env)
    predefinedSymbols.insert(std::make_pair("env", (void *) &g_thread_env)); \
    addExternalObject(*state, (void *) &g_thread_env, 4, true, true, true)->setName("env");
    __DEFINE_EXT_OBJECT_RO(tlb_table);
    __DEFINE_EXT_OBJECT_RO(iotlb);
#if defined(CONFIG_SYMBEX) && defined(CONFIG_SYMBEX_MP)
    __DEFINE_EXT_OBJECT_RO(iotlb_ramaddr);
#endif
//    __DEFINE_EXT_OBJECT_RO(tlb_flush_addr);
//    __DEFINE_EXT_OBJECT_RO(tlb_flush_mask);
#if defined(SE_ENABLE_PHYSRAM_TLB)
    __DEFINE_EXT_OBJECT_RO(se_ram_tlb);
#endif
//    __DEFINE_EXT_OBJECT_RO(se_tlb_current);
    predefinedSymbols.insert(std::make_pair("se_tlb_current", (void *) &se_tlb_current)); \
    addExternalObject(*state, (void *) &se_tlb_current, sizeof(se_tlb_current), false, true, true)->setName("se_tlb_current");
...
void S2EExecutor::registerCpu(S2EExecutionState *initialState, CPUARMState *cpuEnv) {
    ...
    /* Add registers and eflags area as a true symbolic area */
    main_symbolicRegs = addRamExternalObject(*initialState, cpuEnv, offsetof(CPUARMState, regs[15]),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ false,
                                                   /* isSharedConcrete = */ false);
    /* Add the rest of the structure as concrete-only area */
    main_concreteRegs = addExternalObject(*initialState, ((uint8_t *) cpuEnv) + offsetof(CPUARMState, regs[15]),
                                                   sizeof(CPUARMState) - offsetof(CPUARMState, regs[15]),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ true,
                                                   /* isSharedConcrete = */ true);
    initialState->m_registers.initialize(initialState->addressSpace, main_symbolicRegs, main_concreteRegs);
}

void S2EExecutor::registerNewthreadCpu(S2EExecutionState *initialState, CPUARMState *cpuEnv) {
    if (sizeof(*cpuEnv) != cpuEnv->size) {
        std::cerr << "Invalid cpu size structure\n";
        abort();
    }
    /* Add registers and eflags area as a true symbolic area */
    symbolicRegs = addRamExternalObject(*initialState, cpuEnv, offsetof(CPUARMState, regs[15]),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ false,
                                                   /* isSharedConcrete = */ false);
    assert(symbolicRegs);
    /* Add the rest of the structure as concrete-only area */
    concreteRegs = addExternalObject(*initialState, ((uint8_t *) cpuEnv) + offsetof(CPUARMState, regs[15]),
                                                   sizeof(CPUARMState) - offsetof(CPUARMState, regs[15]),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ true,
                                                   /* isSharedConcrete = */ true);
...
void S2EExecutor::registerRam(S2EExecutionState *initialState, MemoryDesc *region, uint64_t startAddress, uint64_t size,
...
#else
        MemoryObject *mo = addRamExternalObject(*initialState, (void *) addr, SE_RAM_OBJECT_SIZE, false,
                                             /* isUserSpecified = */ true, isSharedConcrete,
                                             isSharedConcrete && !saveOnContextSwitch && StateSharedMemory);
	/* This MemoryObject has been registered, continue next*/
	if(!mo) {
		continue;
	}
#ifdef DEBUG_KLEE_MEMORY_BLOCK
    	m_s2e->getDebugStream() << "State = " << initialState->getID() << ":Adding klee memory block (startaddr = " << hexval(hostAddress)
                            << ", size = " << hexval(SE_RAM_OBJECT_SIZE) << ", hostaddr = " << hexval(addr)
    ...
    if (!isSharedConcrete) {
/* Don't unmap RAM in user mode s2e. Why do we even need to allocate new ram memory for klee? */
#ifndef CONFIG_USER_KVM
...
void S2EExecutor::switchToConcrete(S2EExecutionState *state) {
    assert(!state->m_runningConcrete);
    if (PrintModeSwitch) {
        m_s2e->getWarningsStream(state) << "thread = " << hexval((unsigned long)pthread_self()) << " S2EExecutor::switchToConcrete: Switching to concrete execution at pc = " << hexval(state->regs()->getPc())
                                    << '\n';
    }
...
void S2EExecutor::switchToSymbolic(S2EExecutionState *state) {
    ...
    if (PrintModeSwitch) {
        m_s2e->getWarningsStream(state) << "thread = " << hexval((unsigned long)pthread_self()) << " S2EExecutor::switchToSymbolic: Switching to symbolic execution at pc = " << hexval(state->regs()->getPc())
                                    << '\n';
    }
...
void S2EExecutor::doStateSwitch(S2EExecutionState *oldState, S2EExecutionState *newState) {
    ...
    cpu_disable_ticks();
    m_s2e->getWarningsStream(oldState) << "Switching from state " << (oldState ? oldState->getID() : -1) << " to state "
...
S2EExecutionState *S2EExecutor::selectNextState(S2EExecutionState *state) {
    ...
    // memory corruptions.
    if(states.find(nstate) == states.end())
    {
       fprintf(stderr, "No state available, stop\n");
       exit(0); 
    }
...
#ifdef PREPARE_FUNC
    llvm::errs() << "[2] update function = " << kf->function->getName().str() << "\n";
#endif
...
#ifdef PREPARE_FUNC
    llvm::errs() << "[3] function = " << kf->function->getName().str() << "\n";
#endif
    ...
    try {
        while (state->stack.size() != callerStackSize) {
            ...
            if (PrintLLVMInstructions) {
                m_s2e->getWarningsStream(state) << "thread_id = " << hexval((unsigned long)pthread_self()) << " is executing " << ki->inst->getParent()->getParent()->getName().str()
                    << ": " << *ki->inst << '\n';
            }
...
void S2EExecutor::updateConcreteFastPath(S2EExecutionState *state) {
    bool allConcrete = state->regs()->getSymbolicRegistersMask() == 0;	
        std::vector<StackFrame>::const_iterator itA = state->stack.begin();
        while (itA != state->stack.end()) {
            ++itA;
        }
...
void S2EExecutor::updateKleeCpuEnv(CPUArchState *env) {
/* update thread env */
    if(env != g_thread_env) {
        g_thread_env = env;
    /* update  thread register */
        if(symbolicRegs && concreteRegs) 
        {
            g_s2e_state->m_registers.initialize(g_s2e_state->addressSpace, symbolicRegs, concreteRegs);
        } else {
            g_s2e_state->m_registers.initialize(g_s2e_state->addressSpace, main_symbolicRegs, main_concreteRegs);
        }
        g_s2e->getExecutor()->updateConcreteFastPath(g_s2e_state);
    }
}
...
/* As long as there are symbolic registers remaining, we stay in klee symbolic mode. 
The original s2e checks if the current TB will access any symbolic registers and make decision per TB. Mousse chose a more conservertive way */
                    if (1) {
    ...
    if (executeKlee) {
        ...
        assert(state->regs()->getSymbolicRegistersMask() == 0 && "Mousse: All registers have to be concrete before switching to concrete mode\n");
        if (!state->m_runningConcrete)
...
//void S2EExecutor::notifyAlwaysConcreteAccess(ExecutionState &originalState, klee::ref<Expr> &condition,
//                                             bool isWrite) {
//    S2EExecutionState *state = static_cast<S2EExecutionState *>(&originalState);
//    m_s2e->getCorePlugin()->onAlwaysConcreteMemoryAccess.emit(state, condition, isWrite);
//}

void S2EExecutor::notifyForkBeforeBranch(ExecutionState &originalState, klee::ref<Expr> &condition,
                                         Executor::StatePair &targets, int type, bool &swap) {
    ...
    try {
        m_s2e->getCorePlugin()->onStateForkBeforeBranch.emit(state, newStates, newConditions, type, swap);
...
S2EExecutor::StatePair S2EExecutor::fork(ExecutionState &current, klee::ref<Expr> condition, bool isInternal,
                                         bool deterministic, bool keepConditionTrueInCurrentState, bool isSyscallConcretize) {
#ifdef DEBUG_OTHER
    ...
    llvm::raw_ostream &out1 = m_s2e->getWarningsStream(currentState);
    out1 << "Forking state " << currentState->getID() << " at pc = " << hexval(currentState->regs()->getPc())
        << " at pagedir = " << hexval(currentState->regs()->getPageDir()) << '\n';
...
void S2EExecutor::doInterruptArm() {
    g_s2e->getExecutor()->doInterruptArmInternal(g_s2e_state);
}

void S2EExecutor::doInterruptArmInternal(S2EExecutionState *state) {
    if (state->m_registers.allConcrete() && !m_executeAlwaysKlee) {
        if(!state->m_runningConcrete) {
            switchToConcrete(state);
        }
...
void s2e_initialize_execution(int execute_always_klee) {
    ...
    tcg_register_helper_with_reg_mask((void *) &s2e_tcg_execution_handler, "s2e_tcg_execution_handler", 0, 0, 0);
    tcg_register_helper_with_reg_mask((void *) &s2e_tcg_custom_instruction_handler, "s2e_tcg_custom_instruction_handler", 0, 0, 0);
}
...
void s2e_register_newthread_cpu(CPUArchState *cpu_env) {
    g_s2e->getExecutor()->registerNewthreadCpu(g_s2e_state, cpu_env);
}
...
void s2e_register_ram2(const char *name, uint64_t host_address, uint64_t size, int is_shared_concrete) {
    ...
    g_s2e->getCorePlugin()->onRegisterRam.emit(g_s2e_state, host_address, size);
}
...
/* This interface is exposed for Mousse */
uint64_t s2e_solve_suggestedValue_for_concolicVar(klee::ref<klee::Expr> expression){
        return g_s2e_state->solveSuggestedValueForConcolicVar(expression);
}
