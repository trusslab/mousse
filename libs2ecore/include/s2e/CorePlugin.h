...
typedef struct SyscallArguments SyscallArguments;
...
public:
    ...
    ///
    /// Signal emitted when the state is forked but the branch have not been taken.
    /// Use swap to decide the direction of branch.
    ///
    sigc::signal<void,
    ...
    /// Emitted when a ram block is registered
    ///
    sigc::signal<void,
                S2EExecutionState*,
                uint64_t /* address */,
    ...
    /// Emitted before data are concretized being written to always concrete
    /// memory region
    sigc::signal<void,
                S2EExecutionState*,
                klee::ref<klee::Expr> /* value */,
    ...
    /// Emitted before the arguments of syscall are concretized
    ///
    sigc::signal<void,
                S2EExecutionState*,
                int /* is register */,
                uint64_t /* address or reg mask */,
                uint64_t /* size */>
        onConcretizeSyscallArgs;

    /// Emitted before the syscall is handled
    ///
    sigc::signal<void,
                S2EExecutionState*,
                uint64_t /* syscall num */,
                SyscallArguments*>
        onDoSyscallStart;

    /// Emitted after the syscall is handled
    ///
    sigc::signal<void,
                S2EExecutionState*,
                uint64_t /* syscall num */,
    ...
    /// Emitted after the image is loaded
    ///
    sigc::signal<void,
                S2EExecutionState*,
                ImageInfo*>
    ...
    sigc::signal<void,
                S2EExecutionState*,
                uint64_t /* concolic buffer address */,
                unsigned /* concolic buffer size */,
                const std::string & /* concolic buffer name */>
        onOtherMakeConcolic;
