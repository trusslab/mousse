...
protected:
    ...
    StatePair fork(klee::ExecutionState &current, klee::ref<klee::Expr> condition, bool isInternal,
                   bool deterministic = false, bool keepConditionTrueInCurrentState = false, bool isSyscallConcretize = false);
...
public:
    ...
    void registerNewthreadCpu(S2EExecutionState *initialState, CPUARMState *cpuEnv);
#endif
    ...
    static void updateKleeCpuEnv(struct CPUARMState *env);
#endif
    ...
    static void doInterruptArm();
    void doInterruptArmInternal(S2EExecutionState *state);
    ...
    void notifyForkBeforeBranch(klee::ExecutionState &originalState, klee::ref<klee::Expr> &condition, StatePair &targets, int type, bool &swap);
