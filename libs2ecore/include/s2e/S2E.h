...
struct S2EShared {
    ...
    unsigned cleanStates[S2E_MAX_PROCESSES];
    ...
    S2EShared() {
        for (unsigned i = 0; i < S2E_MAX_PROCESSES; ++i) {
            ...
            cleanStates[i] = (unsigned) -1;
        }
    }

    std::mutex stateRevealingMutex;
    pid_t stateRevealingPid;
    unsigned stateRevealingCount;

    bool stateModifyingSyscallCalled;
    bool waitForReboot;
    unsigned active;
...
public:
    ...
    void setCurrentProcessFinished();
    void setActive(bool active);
    bool getActive();
    void setWaitForReboot();
    bool getWaitForReboot();

    bool addStateIfClean(uint32_t state_id);
    bool isStateClean(S2EShared *shared, uint32_t state_id);
    void setNewCleanState(S2EShared *shared, uint32_t state_id);
    void lockStateRevealingMutex(S2EShared *shared);
    void unlockStateRevealingMutex(S2EShared *shared);
    bool callStateModifyingIfClean(uint32_t state_id);
    bool callStateRevealingStartIfClean(uint32_t state_id);
    void callStateRevealingEnd();

    int connectToServer(int port);
    int getDataFromServer(uint32_t opcode, void *data, int size);
    void sendDataToServer(uint32_t opcode, void *data, int size);
    void sendRawDataToServer(void *data, int size);
    int sendQueryToServer(uint32_t opcode, uint64_t query, void *rdata, int size);
};
