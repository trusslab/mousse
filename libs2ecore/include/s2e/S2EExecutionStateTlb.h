...
public:
    ...
    S2EExecutionStateRegisters * getmRegisters() {return m_registers;}
    AddressSpaceCache * getmAsCache() {return m_asCache;}
   ...
   void updateTlbEntryConcreteStatus(struct CPUX86State *env, unsigned mmu_idx, unsigned index,
    ...
    void updateTlbEntryConcreteStatus(CPUARMState *env, unsigned mmu_idx, unsigned index,
                                      const klee::ObjectState *state);
#endif
...
#elif defined(TARGET_ARM)
    void updateTlbEntry(CPUARMState *env, int mmu_idx, uint64_t virtAddr, uint64_t hostAddr);
#endif
    bool audit();
};
}
