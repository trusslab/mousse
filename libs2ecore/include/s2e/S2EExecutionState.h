...
//    thread_local static S2EExecutionStateTlb m_tlb;
    ...
    bool m_runningExceptionEmulationCode;
     /* We maintain the guest brk info per state */
...
public:
    ...
    void writeSyscallSymbolicRegions(uint64_t host_address, uint64_t size);
...
public:
    ...
    inline target_ulong getTargetBrk(int flag) {
	if(flag == 1) {
		return target_original_brk;
	} else if(flag == 2)
	{
		return target_brk;
	}
    ...
    //inline void setTargetBrk(uint64_t source_brk, int flag) {
    inline void setTargetBrk(target_ulong source_brk, int flag) {
	if(flag == 1) {
		target_original_brk = source_brk;
	} else if(flag == 2)
	{
		target_brk = source_brk;
	}
	brk_page = source_brk;
    }
    ...
    virtual uint64_t multiConcretize(klee::ref<klee::Expr> expression, const std::string &reason);
/* This interface is exposed for Mousse */
    uint64_t solveSuggestedValueForConcolicVar(klee::ref<klee::Expr> expression);
